...
// loginProcessingUrl("/login") /login으로 요청이 오면 자동으로 loadUserByUsername 함수가 실행
@Service
public class PrincipalDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        if (userEntitiy != null) {
            return new PrincipalDetails(userEntitiy); // 반환값은 security session으로 <=(Authentication <= UserDetails)
        }
        return null;
    }
}
